            ...
            for query in Stech.object_to_json(query_results):
                if query['nsat']:
                    if query['nsat'] > 3:
                        ...
                        Logger.add_to_log("error", 'Cantidad de satélites insuficientes para enviar punto', load_data('LOG_DIRECTORY'), "log_waypoint")
                else:
                    Logger.add_to_log("error", 'Punto sin satélite', load_data('LOG_DIRECTORY'), "log_waypoint")
